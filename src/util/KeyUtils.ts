...
export function isEd25519JWK(jwk: JWK): boolean {
  return jwk && jwk.crv && jwk.crv === DidAuth.KeyCurve.ED25519;
}
...
export function verifyJWTSignatureFromVerificationMethods(
  jwt: string,
...
export function getThumbprint(hexPrivateKey: string, method: string): string {
  return getThumbprintFromJwk(
    isEd25519DidKeyMethod(method)
      ? getPublicJWKFromDIDHexPrivateKey(hexPrivateKey)
