...
describe('presentation exchange manager tests', () => {
    it('validatePresentationAgainstDefinition: should throw error if provided VP doesn\'t match the PD', async function () {
        ...
        try {
            await PresentationExchange.validatePresentationAgainstDefinition(verifiedJwt.presentationDefinitions[0].definition, new VP(new Presentation(null, null, null, vcs, null, null)));
        } catch (e) {
    ...
    it('validatePresentationAgainstDefinition: should pass if provided VP match the PD', async function () {
        ...
        const result = await PresentationExchange.validatePresentationAgainstDefinition(verifiedJwt.presentationDefinitions[0].definition, new VP(new Presentation(null, null, null, vcs, null, null)));
    ...
    it('submissionFrom: should pass if a valid presentationSubmission object created', async function () {
        ...
        const vcs = await getVCs();
        const pd: PresentationDefinitionWithLocation[] = PresentationExchange.findValidPresentationDefinitions(payload);
        await PresentationExchange.validatePresentationAgainstDefinition(pd[0].definition, new VP(new Presentation(null, null, null, vcs, null, null)));
    ...
    it('should not validate a list of VerifiablePresentations against a list of PresentationDefinitions', async () => {
        ...
        await expect(PresentationExchange.validatePayloadsAgainstDefinitions(pd, [vpw])).rejects.toThrow(Error('This type of verifiable presentation isn\'t supported in this version'));
    });
});
