...
import { DID_ION_DOCUMENT, DID_ION_ORIGIN, DID_KEY_DOCUMENT, DID_KEY_ORIGIN } from '../data/mockedData';
...
describe('validateLinkedDomainWithDid', () => {
  it('should issue a configuration resource', async () => {
    ...
    await expect(issuer.issueDidConfigurationResource(args)).resolves.toEqual(
        ...
        linked_dids: [
          {
            ...
            expirationDate: '2032-09-02T22:00:00.000Z',
            ...
            proof: {
              ...
              verificationMethod: 'did:key:z6MkoTHsgNNrby8JzCNQ1iRLyW5QQ6R8Xuu6AA8igGrMVPUM',
            },
  ...
  it('should issue a linked domain credential', async () => {
    const issueCallback = async (args: IIssueCallbackArgs): Promise<any> => {
      const suite = new Ed25519Signature2020({ key: keyPair });
      suite.verificationMethod = args.credential.credentialSubject.id;
      const documentLoader = new DocumentLoader();
    ...
    dlc = await issuer.issueDomainLinkageCredential(args);
    expect(dlc).toEqual(
      expect.objectContaining({
        '@context': [
          'https://www.w3.org/2018/credentials/v1',
  ...
  it('should verify a link domain credential', async () => {
    const verifySignatureCallback = async (args: IVerifyCallbackArgs): Promise<IVerifyCredentialResult> => {
      const suite = new Ed25519Signature2020({ key: keyPair });
      const documentLoader = new DocumentLoader();
      return await vc.verifyCredential({ credential: args.credential, suite, documentLoader: documentLoader.getLoader() });
    };
  ...
  it('should succeed with key did and CheckLinkedDomain.ALWAYS', async () => {
    ...
    await expect(
      validateLinkedDomainWithDid('did:key:z6MkoTHsgNNrby8JzCNQ1iRLyW5QQ6R8Xuu6AA8igGrMVPUM', verifyCallbackTruthy, CheckLinkedDomain.ALWAYS)
