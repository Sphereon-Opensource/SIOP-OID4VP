...
nock(URL).post('/200', {iss: "mock"}, {reqheaders: {Authorization: "Bearer bearerToken"}}).reply(200, '{"status": "ok"}');
nock(URL).post('/201', {iss: "mock"}, {reqheaders: {Authorization: "Bearer bearerToken"}}).reply(201, '{"status": "ok"}');
