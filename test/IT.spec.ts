...
describe('RP and OP interaction should', () => {
  ...
  it('succeed when calling with LinkedDomainValidationMode.ALWAYS', async () => {
    ...
    const rp = RP.builder()
      .withCheckLinkedDomain(CheckLinkedDomain.ALWAYS)
      .withRevocationVerification(RevocationVerification.IF_PRESENT)
  ...
  it('should verify jsonld vcs with valid revocation', async () => {
    const vpToken = {
      "presentation": {
        "@context": [
    ...
    await expect(
        verifyRevocation(vpToken, async () => { return { status: RevocationStatus.VALID } })
    ).resolves.not.toThrow();
  });

  it('should verify jwt vcs with valid revocation', async () => {
    const vpToken = {
      "presentation": {
        "@context": [
          "https://www.w3.org/2018/credentials/v1",
